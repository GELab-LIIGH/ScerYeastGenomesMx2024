...
def find_orthologs(sapaf,sacef):
	seqIterSACE = SeqIO.parse(fastaSACE, 'fasta')
	seqIterSAPA = SeqIO.parse(fastaSAPA, 'fasta')
		...
		for gene in geneName:
			...
			if True:
				...
				if SACEorthID != "NA":	#The next 2 lines are new, avoid miscalculationg Seq_ID
					...
					for seqE in seqIterSACE:
						if seqE.id == SACEorthID:
							SACEseq = str(seqE.seq)
					for seqA in seqIterSAPA:
						if SAPAorthID == seqA.id.split("|")[0]:
							SAPAseq = str(seqA.seq)
					if SACEseq != "NA":
